...
const BRANCH    = process.env.TARGET_BRANCH || 'HOLLYWOOD';
...
// ─── 메인 ─────────────────────────────────────────────────────
async function main() {
  console.log(`[daily-tasks] BRANCH=${BRANCH}, ET date=${todayInBranch()}`);
  ...
  for (const t of TASKS) {
    const url = `${FB_DB}/tasks/${BRANCH}/${today}/${t.taskId}.json?auth=${encodeURIComponent(token)}`;
    try {
      // 1) 이미 있으면 skip (직원 진행 데이터 보존)
      ...
      if (w.ok) {
        console.log(`  [${t.key}] ✅ 새로 발송 — ${t.taskId}`);
        created++;
      } else {
        const body = await w.text().catch(() => '');
        console.error(`  [${t.key}] ❌ PUT 실패 HTTP ${w.status}: ${body.slice(0, 120)}`);
        failed++;
      }
    } catch (e) {
      console.error(`  [${t.key}] ❌ 예외:`, e.message);
      failed++;
    }
  }
